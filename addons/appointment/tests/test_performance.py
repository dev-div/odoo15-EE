...
class AppointmentTest(AppointmentPerformanceCase):
    ...
    def test_get_appointment_slots_custom(self):
        ...
        with self.mockAppointmentCalls(), \
             self.assertQueryCount(staff_user_bxls=274):  # apt only: 271
            t0 = time.time()
        ...
        # Method count before optimization: 240 - 240 - 240 - 1

        slot_startdate = date(2022, 1, 31)  # starts on a Monday, first week containing Feb day
        slot_enddate = date(2022, 4, 3)  # last day of last week of May
        self.assertSlots(
            res,
            ...
            {'enddate': slot_enddate,
             'startdate': slot_startdate,
            }
        )
    ...
    def test_get_appointment_slots_website_whours(self):
        ...
        # with self.profile(collectors=['sql']) as profile:
        with self.mockAppointmentCalls(), \
             self.assertQueryCount(staff_user_bxls=483):  # apt only: 459
            t0 = time.time()
        ...
        # Method count before optimization: 361 (-3 run) - 358 (-3 run) - 358 (-3 run) - 20

        slot_startdate = date(2022, 1, 31)  # starts on a Monday, first week containing Feb day
        slot_enddate = date(2022, 5, 1)  # last day of last week of April
        self.assertSlots(
            res,
            [{'name_formated': 'February 2022',
              'weeks_count': 5,  # 31/01 -> 28/02 (06/03)
    ...
    def test_get_appointment_slots_website_whours_short(self):
        ...
        # with self.profile(collectors=['sql']) as profile:
        with self.mockAppointmentCalls(), \
             self.assertQueryCount(staff_user_bxls=199):  # apt only: 173
            t0 = time.time()
            res = apt_type._get_appointment_slots('Europe/Brussels', reference_date=self.reference_now)
            t1 = time.time()
        ...
        # Method count before optimization: 75 (-1 run) - 72 (-1 run) - 72 (-1 run) - 20

        slots_startdate = date(2022, 1, 31)  # starts on a Monday, first week containing Feb day
        slots_enddate = date(2022, 3, 6)  # last day of last week of Feb
        self.assertSlots(
            res,
            [{'name_formated': 'February 2022',
              'weeks_count': 5,  # 31/01 -> 28/02 (06/03)
    ...
    def test_get_appointment_slots_work_hours(self):
        ...
        # with self.profile(collectors=['sql']) as profile:
        with self.mockAppointmentCalls(), \
             self.assertQueryCount(staff_user_bxls=967):  # apt only: 967 (runbot: 927 ?)
            t0 = time.time()
            res = apt_type._get_appointment_slots('Europe/Brussels', reference_date=self.reference_now)
            t1 = time.time()
        ...
        # Method count before optimization: 911 (- 41) - 910 (- 41) - 910 (- 41) - 1

        slot_startdate = date(2022, 1, 31)  # starts on a Monday, first week containing Feb day
        slot_enddate = date(2022, 6, 5)  # last day of last week of May
        self.assertSlots(
            res,
            [{'name_formated': 'February 2022',
              'weeks_count': 5,  # 31/01 -> 28/02 (06/03)
    ...
    def test_get_appointment_slots_work_hours_short(self):
        ...
        # with self.profile(collectors=['sql']) as profile:
        with self.mockAppointmentCalls(), \
             self.assertQueryCount(staff_user_bxls=169):  # apt only: 169 (runbot: 161 ?)
            t0 = time.time()
            res = apt_type._get_appointment_slots('Europe/Brussels', reference_date=self.reference_now)
            t1 = time.time()
        ...
        _logger.info('Called methods\nSearch calendar event called %s\n'
                     ...
                     self._mock_cal_work_intervals.call_count)
        # Time before optimization: ~0.30
        # Method count before optimization: 113 (-9 run) - 112 (-9 run) - 112 (-9 run) - 1
...
class OnlineAppointmentPerformance(AppointmentUIPerformanceCase):
    ...
    def test_appointment_type_page_website_whours_user(self):
        ...
        with freeze_time(self.reference_now):
            ...
            with self.assertQueryCount(default=460):  # apt only: 451 (457 with website_apt)
